...
def _sample_dwell() -> pd.DataFrame:
    return pd.DataFrame(
        {
            "farm_id": ["Alpha", "Alpha", "Alpha", "Bravo", "Bravo"],
            "dwell_tier": ["Tier A", "Tier B", "Tier D", "Tier A", "Tier B"],
            "duration_min": [30.0, 150.0, 300.0, 200.0, 90.0],
            "mmsi": [111, 222, 333, 111, None],
            "possible_cross_farm_duplicate": [True, False, False, True, False],
            "duplicate_group_id": ["dup-1", None, None, "dup-1", None],
        }
    )
...
def test_manifest_denominator_counts_observed_zero_and_excludes_missing_source():
    denominator = build_manifest_denominator(_sample_manifest()).set_index("farm_id")

    assert denominator.loc["Alpha", "observed_months"] == 2
    assert denominator.loc["Alpha", "success_no_ais_in_bbox_months"] == 1
    assert denominator.loc["Alpha", "skipped_missing_source_months"] == 1
    assert denominator.loc["Alpha", "observed_years"] == 2 / 12
...
def test_output_schema_and_observed_zero_rate_behaviour():
    ...
    assert by_farm.loc["Charlie", "confidence_class"] == "no_observed_coverage"
...
def test_build_rq9_farm_outputs_writes_expected_files(tmp_path: Path):
    ...
    assert outputs.validation["observed_years_total"] == 4 / 12
