...
def build_rq_readiness_matrix(layer_status: dict[str, bool]) -> pd.DataFrame:
    ...
    rq_specs = [
        ...
        },
        {
            ...
            "geo": "medium",
            "obs": "medium",
            "action": "Proceed only with source-aware metocean sensitivity/readiness work; keep wind direction and current coverage limitations explicit.",
        },
        {
            "rq_number": "RQ7",
...
def build_report_text(
    ...
    lines = [
        ...
        "",
        "- Source of truth: `scripts/build_evidence_readiness.py` calls `src/om_pipeline/analysis/evidence_readiness.py` from the repository root.",
        "- Rebuild command: `/opt/anaconda3/bin/python scripts/build_evidence_readiness.py`.",
        "- The matrices under `Data/Processed/analysis/evidence_readiness/` are derived outputs and may be ignored by git; regenerate them from the existing local source artifacts instead of hand-editing or committing them.",
        "- The tracked report files under `reports/evidence_readiness/` are generated by the same builder and should be refreshed after any source or input change.",
        "",
        "## Coverage Findings",
        "",
        ...
        "- `success_no_ais_in_bbox` is observed zero-event evidence, not missing source evidence.",
        "- `skipped_missing_source` is missing source evidence, not zero-event evidence.",
        "- Missing wind direction, current, and vessel-enrichment values remain missing/null in feature layers and false only as readiness flags; they are never imputed to zero physical values or empty enrichment.",
        "",
        ...
        "- RQ6 is ready only for source-aware metocean sensitivity/readiness work; it is not a calibrated access-probability claim.",
        "- RQ9 remains blocked for failure claims until direct AIS receiver/source geometry and fault/work-order validation exist.",
        "",
        rq_table,
        "",
        "## Guardrails",
        ...
        "- Do not treat missing vessel enrichment as an unknown-but-present registry class.",
