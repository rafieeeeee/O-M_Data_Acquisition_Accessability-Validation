...
| --- | --- | --- | --- | --- |
| RQ01 Stage 2 Fusion v2 workability sensitivity | Implementation active / pending review | Do wind speed and event-scale current materially change the observed Tier A workability envelope relative to wave-only Fusion v1/Fusion v2 evidence? | Observed-envelope sensitivity only; no calibrated access-probability claim. Current-aware comparisons are NWS-domain / coverage-limited, wind direction is excluded, and depth-warning sensitivity is required. | Review restricted descriptive sensitivity result; if accepted, merge and update status to `Result accepted`. |
