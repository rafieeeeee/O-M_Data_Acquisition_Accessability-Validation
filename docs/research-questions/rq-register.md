...
| --- | --- | --- | --- | --- |
| RQ01 Stage 2 Fusion v2 workability sensitivity | Spec drafted | Do wind speed and event-scale current materially change the observed Tier A workability envelope relative to wave-only Fusion v1/Fusion v2 evidence? | Observed-envelope sensitivity only; no calibrated `P(operation | weather)` claim. Current-aware comparisons are NWS-domain / coverage-limited, wind direction is excluded, and depth-warning sensitivity is required. | Review `docs/research-questions/rq01_stage2_workability_sensitivity/analysis-plan.md` before implementation. |
